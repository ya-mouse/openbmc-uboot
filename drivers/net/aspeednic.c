...
#if ! defined(CONFIG_AST2100) && (defined(CONFIG_AST2300_FPGA_2) || defined(CONFIG_AST2300) || defined(CONFIG_AST3100) || defined(CONFIG_AST2400))
...
int aspeednic_initialize(bd_t *bis)
{
  ...
  *(volatile u_long *)(SCU_BASE) = 0x1688A8A8;
...
static void aspeednic_probe_phy(struct eth_device *dev)
{
  ...
  for (phy_addr = 0; phy_addr <= 0x1f; phy_addr++) {
    phy_id = phy_read_register(dev, 0, phy_addr);
    /*
